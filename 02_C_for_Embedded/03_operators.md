...
Question:
Does `&a` mean the value?
No. It means: "Give me the memory address where a is stored."
So: `&a` returns: 1000
...
Read it in English: Store the address of a into ptr.
We are not storing 10. We are storing: 1000
...
The pointer ptr has its own memory location. Its value is an address.
...
#### Why is this useful?
Imagine you have a huge structure:
```c
struct SensorData sensor;
```
Instead of copying the entire structure, we simply pass: `&sensor` which is just an address.
This is much faster and is one reason pointers are used extensively in embedded systems.
...
What does `*ptr` mean?
Read it as:
Go to the address stored in ptr and give me the value stored there.
...
`*ptr` means:
"Follow the arrow and read what's there."

##### CPU View (Embedded Level)
Suppose: `int value = *ptr;`
Conceptually, the CPU may execute something like:
```asm
LDR R1, [ptr]      ; R1 = address stored in ptr (3000)
LDR R2, [R1]       ; R2 = value at address 3000 (25)
STR R2, [value]    ; Store 25 into value
```

Notice something important:
The CPU first reads the address from ptr.
Then it uses that address to read the actual data.
Registers are still used as temporary storage, just as in the arithmetic examples.

##### Changing a value through a pointer
Pointers don't just read values—they can also modify them.
...
We didn't write: `a = 100;`
But because ptr points to a, changing `*ptr` changes a.
...
Questions
What is the value of ptr?  4000
What is the value of *ptr? 50
...
```c
float voltage = 3.3;
int value = (int)voltage;
```

Here: 3.3 is converted to 3
...
Notice:
The value didn't change. Only its representation changed.
...
Convert integer to floating-point format
       ↓
Store floating-point value in RAM
```

The compiler generates the appropriate instructions (or library support, depending on the target and operation). We don't manually perform the conversion.

The important idea is:
Arithmetic (+, -, etc.) happens in the ALU.
Type conversion is performed as directed by the instructions generated by the compiler.

#### Embedded Example
Suppose an ADC returns:
```c
uint16_t adc = 2048;
```
...
Why cast?
Without the cast, integer arithmetic could be performed first, giving the wrong result. Casting tells the compiler to perform the calculation using floating-point arithmetic.
...
The compiler automatically converts 10 to 10.0.
...
You explicitly tell the compiler to convert the value.
...
The cast changes how the value is interpreted for that expression. It does not change the original variable.
...
Think of precedence as the order in which operators are evaluated.
...
### C Operator Precedence (Embedded-Friendly)
Highest → Lowest priority

| Level | Operators | Description | Associativity |
|-------|-----------|-------------|---------------|
| 1 | () [] -> . | Function call, array index, struct access | Left → Right |
| 2 | ++ -- (postfix) | Post increment/decrement | Left → Right |
| 3 | ++ -- + - ! ~ * & (type) sizeof | Unary operators | Right → Left |
| 4 | * / % | Multiply, divide, modulus | Left → Right |
| 5 | + - | Add, subtract | Left → Right |
| 6 | << >> | Bit shifts | Left → Right |
